...
var App = (function() {
  ...
  p.start = function() {
    ...
    this.server.on('request', function(request, response) {
      ...
      var sessionService;
      var cookieService = new CookieService(that);
      ...
      var doParseReq = function() {
        ...
        isHtml = (extname === '.html');
        if (isHtml || extname === '.json') {
          promise = requestHandler.handle();
          sessionService = new SessionService(that);
          serviceManager.setService({
            key: '_SES',
      ...
      var doLoadTemplate = function(req) {
        ...
        // TODO:セッションオブジェクトを引き渡す
        promise = responseBuilder.load(templatepath, mtime, resultForReq.reqid, {
          'auth_login_id': '12345',
          'auth_login_group': ['admin', 'hws']
        });
        return promise;
      };
      // 【手順.03】リクエスト　→　データストア　→　処理結果
      var doCrud = function(template) {
        var promise;
        ...
        if (DEFAULT_METHODS.indexOf(parsedRequest._method)) {
          ...
          parsedRequest = $.extend(parsedRequest, {
            '_LOGIN_INFO': {
              'owner': '12345',
              'group': ['admin999', 'admin1']
            }
          });
          // セッションIDを保持（ResponseBuilderで参照するため）
          if (isHtml) {
            cookieService.cookieDao.set({
              'wao-session-id': sessionService.getSessionId()
            });
      ...
      var doResponse = function(result) {
        ...
        resultForReq.reqid = requestHandler.reqid;
        ...
        if (isHtml) {
          resultForReq._COOKIE = { // cookieに関するhttpヘッダ情報
            '_header': {
              'Set-Cookie': cookieService.cookieDao.getHeaderStrList()
            }
          };
        }
        responseBuilder.build(tmp, resultForReq);
        if (isHtml) {
          sessionService.persistent(); // セッション情報をDBに保存
        }
        d.resolve();
        return d.promise();
      };
      ...
      $.Deferred().resolve()
        ...
        .then(doCrud) // 【手順.03】リクエスト　→　データストア　→　処理結果
        .then(doCrudShort) // 【手順.04】
  ...
  p.openDb = function() {
    ...
    this.mongoDb.open(function(err) {
      ...
      that.logger.info('Connected to mongoDb. name = ' + that.getAppId());
    });
  };
