...
var App = (function() {
  ...
  function openDb(mongoDb) {
    mongoDb = new mongo.Db(
      getAppId(), new Server('localhost', mongo.Connection.DEFAULT_PORT, {}));

    mongoDb.open(function(err, db) {
      if (err !== null) {
        logger.info('Success to open mongoDb connection');
        return;
      }
      logger.error('Error to open mongoDb connection');
    });
  }
