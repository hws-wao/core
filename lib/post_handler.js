...
var PostHandler = (function() {
  ...
  p.handle = function() {
    ...
    return function(){
      ...
      if (that.request.headers['content-type'].indexOf('multipart/form-data') >= 0) {
        // (new FileUploader({ app: that.app, request: that.request })).upload();
  ...
  //        $.when(thata.fileUploade(thata, data, result), thata.postData(fields, result))
          $.when(thata.fileUploade(thata, data, result))
        ...
        switch (query[key]){
          case 'start()':
            ...
            if (appProp[0] != 'undefined' && appProp[1] != 'undefined') {
              this.app.wao.createAndStartApp({ name: appProp[0] , port: appProp[1]  });
            }
            break;
          default :
            break;
