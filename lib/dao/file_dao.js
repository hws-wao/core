...
var FileDao = (function() {
  ...
  p.stat = function(args) {
    ...
    fileStat.path = args.path;
    fileStat.ext = path.extname(fileStat.path);
    fileStat.mimeType = mime.lookup(fileStat.ext);
  ...
  p.mvFiles = function(args) {
    ...
    var mvFile = args.mvFile;
    var fncEntry = function(entry) {
      var data = '';
      entry.on('data', function(chunk) {
        data += chunk;
      });
      entry.on('end', function(callback) {
        callback(null, data);
      });
    };
    var fncMvFile = function(err, data) {
    ...
    if (path.extname(mvFile.from) === '.zip' && path.extname(mvFile.to) !==
      ...
      if (mvFile.to.match(/\/$/)) {
        fs.readFile(mvFile.from, fncMvFile);
      } else {
        mv(mvFile.from, to, {
          mkdirp: true
        }, fncResolve);
  ...
  p.createReadStream = function(args) {
    var filePath = args.path;
    this.checkRequiredArg(filePath, 'path');

    var filePathAbs = path.resolve(path.join(this.baseDir, filePath));

    var rStream = fs.createReadStream(filePathAbs);
    return rStream;
  };
