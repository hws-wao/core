...
var CookieDao = (function() {
  ...
  var CookieDao = function(args) {
    ...
    this.cookieObj = cookie.parse(args.request.headers.cookie);
  ...
  p.remove = function(args) {
    ...
    if (!Array.isArray(args)) {
      throw '/wao/lib/model/dao/cookie_dao.js: remove(): args is not array.';
    }
  ...
  p.get = function(args) {
    ...
    args = (typeof args === 'string') ? [args] : args;
    if (!Array.isArray(args)) {
      throw '/wao/lib/model/dao/cookie_dao.js: get(): args is not array.';
    }
    // Cookieから情報を取得
    for (var i = 0; i < args.length; i++) {
      var key = args[i];
      ...
      // 値をチェック
      if (!value) {
        throw '/wao/lib/model/dao/cookie_dao.js: get(): cookie is not found. key=' + key;
      }
      // Result作成
      res[key] = value;
      // HTTPヘッダ値情報は作成しない
