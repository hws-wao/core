...
var File = (function() {
  ...
  p.read = function(args) {
    ...
    checkPermission(this.baseDir, filePath)
      ...
      .then(isFile)
      .then(readFile)
      .done(function(html) { d.resolve(html); })
      .fail(function(e) {
        d.reject(new AppError(e.status, null, { message: e.message }));
      });
  ...
  p.mkdirp = function(args) {
    ...
    if (!this.checkPermission(filePathAbs)) { throw new Error('Permission Error. [' + dirPath + ']') }
  ...
  p.mvFiles = function(args) {
    ...
    for (var i = 0; i < mvFileList.length; i++) {
      ...
      if (!this.checkPermission(to)) { throw new Error('Permission Error. [' + to + ']') }
      if (path.extname(mvFile.from) === '.zip' && path.extname(mvFile.to) !== '.zip') {
        rimraf.sync(to);
        ...
        mv(mvFile.from, to, { mkdirp: true }, function() { d.resolve(); });
      }
  ...
  function checkPermission(baseDir, filePath) {
    var d = $.Deferred();
    _.startsWith(filePath, baseDir) ?
        d.resolve(filePath)
      : d.reject({ status: 403, message: 'Forbidden' });
    return d.promise();
  }
  ...
  function isExists(filePath) {
    ...
    fs.exists(filePath, function(exists) {
      exists ?
          d.resolve(filePath)
        : d.reject({ status: 404, message: 'Not Found' });
    });
    return d.promise();
  }
  ...
  function readFile(filePath) {
    var d = $.Deferred();
    fs.readFile(filePath, { encoding: this.defaultEncoding }, function(err, data) {
      if (err) { d.reject(err); }
      else { d.resolve(data); }
    });
    return d.promise();
