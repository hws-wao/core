...
var ResponseBuilder = (function() {
  ...
  p.build = function(result) {
    ...
    var mimeType;
    if (result._FILE.ext) {
      mimeType = mime.lookup(result._FILE.ext);
    }
  ...
  p.directResponse = function() {
    var mimeType = mime.lookup(this.request.url);
    var parsedUrl = url.parse(this.request.url);
    var filePath = this.app.file.baseDir + '/templates' + parsedUrl.pathname;

    if (this.app.file.checkPermission(filePath)) {
      this.response.setHeader('Content-Type', mimeType);
