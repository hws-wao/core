...
var GetHandler = (function() {
  ...
  p.loadTemplate = function(loadPath, result) {
    ...
    (function() {
      ...
      if (that.parsedQuery['_FILE'] && 'path' in that.parsedQuery['_FILE']) {
        that.app.file.readDirRecursive(that.app.file.baseDir + '/apps' + that.parsedQuery._FILE.path + 'templates/', function(fileList) {
          _d.resolve(fileList);
        });
      } else {
        _d.resolve();
