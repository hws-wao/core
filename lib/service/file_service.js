...
var FileService = (function() {
  ...
  function loadFiles(fileDao, dbDao, fileParams, meta, resolve, reject) {
    var result = {};
    var promises = [];
    parseLoadFileParams(fileDao, dbDao, fileParams, null, meta, promises, result);
    $.when.apply(null, promises).done(function(result) {
      resolve(result);
    }).fail(reject);
  }
   ...
   * @param {object} 返却するファイル情報を格納するオブジェクト
   */
  function parseLoadFileParams(fileDao, dbDao, fileParams, path, meta, promises, result) {
    Object.keys(fileParams).forEach(function(key) {
      var _path = _.isEmpty(path) ? key : path + '.' + key;
      var val = fileParams[key];
      if (typeof val === 'object') {
        parseLoadFileParams(fileDao, dbDao, fileParams[key], _path, meta, promises, result);
      } else {
        promises.push(loadFileAsync(fileDao, dbDao, path, key, meta, result));
      }
    });
  }
   ...
   * @param {object} 返却するファイル情報を格納するオブジェクト
   */
  function loadFileAsync(fileDao, dbDao, path, key, meta, result) {
    var d = $.Deferred();
    checkAuth(dbDao, '..' + path, meta['_LOGIN_INFO'], 'r')
      .done(function() {
        if (key === '_body' || key === '_path') {
          if (!result[path]) {
            result[path] = {};
            result[path][key] = "";
          }
        ...
        if (key === '_body') {
          // ファイルの内容を取得
          result[path][key] = fileDao.read({
            path: path
          });
        } else if (key === '_path') {
          // 公開URLのパスを取得
          if (path.match(/^\/templates\//)) {
            result[path][key] = path.replace(/^\/templates/, '');
          } else {
            var hp = new HiddenPath();
            result[path][key] = hp.encrypt(path);
          }
        } else {
          // ディレクトリ構造取得
          result[key] = fileDao.readDirRecursive(key);
        }
        d.resolve(result);
      })
      .fail(function(e) {
