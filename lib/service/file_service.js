...
var FileService = (function() {
  ...
  function removeFiles(fileDao, dbDao, files, meta, resolve, reject) {
    files.forEach(function(path, i) {
      checkAuth(dbDao, '..' + path, meta['_AUTH'], 'w')
        .done(function() {
          fileDao.unlink({
            path: path
          });
  ...
  function doUploadFiles(fileDao, dbDao, reqArgs, meta, resolve, reject) {
    ...
    var mvFileList = createFileFromToPathList(reqArgs);

    var promises = [];
    for (var i = 0; i < mvFileList.length; i++) {
      promises.push(uploadFile(fileDao, dbDao, meta, mvFileList[i]));
    }
    $.when.apply(null, promises).done(resolve).fail(reject);
  }

  function uploadFile(fileDao, dbDao, meta, mvFile) {
    var d = $.Deferred();
    checkAuth(dbDao, mvFile.to, meta['_AUTH'], 'w')
      .then(function() {
   ...
   */
  function loadFiles(fileDao, dbDao, fileParams, path, meta, resolve, reject) {
    var files = {};
    Object.keys(fileParams).forEach(function(key) {
      ...
      if (typeof val === 'object') {
        val = loadFiles(fileDao, dbDao, fileParams[key], _path, meta, resolve, reject);
      } else {
        checkAuth(dbDao, '..' + path, meta['_AUTH'], 'r')
          .done(function() {
            if (key === '_body') {
              // ファイルの内容を取得
  ...
  function checkAuth(dbDao, path, auth, rw) {
    ...
    .done(function(collectionName, docs) {
        if (docs.length > 0) {
          // pathは一意なので、かならず1件
          if (!authUtils.checkAuth(docs[0], auth, rw)) {
  ...
  p.get = serviceFunc(function(d, reqParams, meta) {
    loadFiles(this.fileDao, this.dbDao, reqParams, null, meta, resolve(d), reject(d));
  });
