...
var GetHandler = (function() {
  ...
  p.loadFile = function(loadPath, result) {
    var d = $.Deferred();
    var hiddenPath = new HiddenPath();
    if(hiddenPath.isHiddenPath(loadPath.replace('templates', ''))) {
      var decryptPath = hiddenPath.decrypt(loadPath.replace('templates', ''));
      result._FILE.binary = fs.readFileSync(this.app.file.makeAbsolutePath(decryptPath));
    } else {
