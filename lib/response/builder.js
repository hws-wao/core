...
var ResponseBuilder = (function() {
   ...
   * 指定パスのファイルが見つからなかった場合は404情報を返却する。<br>
   *　また、アクセス対象のファイルに参照権限があるかどうか確認し、
   *　権限がなかった場合は403を返却する。
   *
   ...
   */
  ResponseBuilder.prototype.load = function(filepath, mtime, reqid, session) {
    var d = $.Deferred();
    var that = this;
    var template;
    // ファイルの属性情報を取得する
  ...
  ResponseBuilder.prototype.build = function(template, result) {
    ...
    if (template.mimeType === MIME_HTML && template.status === 200) {
      ...
      // パターン(3)：jsonの返却
      var json = this.buildJson(template, result);
      this.responseText({
   ...
   */
  ResponseBuilder.prototype.buildJson = function(template, buildParam) {
    if (template.path.match(/^\/_DB/)) {
      if (buildParam._DB) {
        template.status = 200;
        return JSON.stringify(buildParam._DB);
      }
    } else if (template.path.match(/^\/_FILE/)) {
      if (buildParam._FILE) {
        template.status = 200;
        return JSON.stringify(buildParam._FILE);
      }
    }
    template.status = 404;
    return '';
  ...
  /**
   * 当該画面へのアクセス権限があるかチェックする
   *
   * @param {object} template テンプレート
   * @param {object} template.jqueryObj バインド前jQueryオブジェクト
   * @param {object} session セッションオブジェクト
   * @returns {boolean} アクセス権限がある場合、true
   */
  function checkAuth(template, session) {
    if (getAllow(template) === 'signed') {
      if (!session) {
        // セッションが存在しない場合、権限チェックエラーとする
        return false;
      }

      // 'signed'が指定されていた場合、セッションオブジェクトのログインIDの有無をチェックする
      // TODO:セッションKEYは、CONFIGで設定した値を参照する
      if (session['auth_login_id']) {
        // セッションにログインIDが存在していれば、ログイン済みと判断する
        return true;
      } else {
        // セッションにログインIDが存在しない場合、権限チェックエラーとする
    ...
    var allowGroup = getAllowGroup(template);
    if (allowGroup) {
      if (!session) {
        // セッションが存在しない場合、権限チェックエラーとする
        return false;
      ...
      // 'signed'以外が指定されていた場合、group名が指定されたと判断する
      ...
      if (session['auth_login_group']) {
        ...
        if (Array.isArray(loginGroup)) {
          for (var i = 0; i < allowGroup.length; i++) {
            if ($.inArray(allowGroup[i], loginGroup) >= 0) {
              return true;
            }
          }
          return false;
        } else {
          if (loginGroup === allowGroup) {
            return true;
          } else {
            return false;
          }
      ...
      // wao-allow,wao-allow-groupが指定されていない場合、チェックしない
      return true;
    }
  }

  /**
   * wao-allowに設定された値を取得する<br>
   *
   * @param {object} template テンプレート
   * @param {object} template.jqueryObj バインド前jQueryオブジェクト
   * @returns {string} signed:ログイン済みユーザのみアクセス可
   */
  function getAllow(template) {
    var $$ = template.jqueryObj;
    var $$html = $$('html').clone();

    var allow = $$html.find('meta[name="wao-allow"]').attr('content');

    if (allow) {
      return allow.toLowerCase();
    }
    return null;
  }

  /**
   * wao-allow-groupに設定された値を取得する<br>
   *
   * @param {object} template テンプレート
   * @param {object} template.jqueryObj バインド前jQueryオブジェクト
   * @returns {string} アクセスを許可するグループ名
   */
  function getAllowGroup(template) {
    var $$ = template.jqueryObj;
    var $$html = $$('html').clone();

    var allowGroup = $$html.find('meta[name="wao-allow-group"]').attr('content');

    if (allowGroup) {
      return allowGroup.toLowerCase().split(",");
    }
    return null;
  }
