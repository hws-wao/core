...
var RequestHandler = (function() {
  ...
  p.handle = function() {
    ...
    if (this.request.method === 'GET') {
      ...
      ret = $.extend(ret, parseRequestPath(this.request, ret));
      d.resolve(ret);
      return d.promise();
    } else
   ...
   * 
   * @param {request} HTTPリクエストオブジェクト
   * @param {object} params GETパラメタ（クエリストリング）の解析結果
   * @returns {object} WAOアプリ用リクエストセット
   */
  function parseRequestPath(request, params) {
    var pathname = url.parse(request.url).pathname;
    var extname = path.extname(pathname);
    if (extname === '.json') {
      var pathArr = pathname.split('.');
      if (pathArr[0].toUpperCase() === '/_DB') {
        if (!params['_DB']) {
          params['_DB'] = {};
        }
        if (pathArr.length === 3) {
          // _DB.{collection}.json or _DB._collections.json
          var collection = pathArr[1];
          if (collection === '_collections') {
            params['_DB']['_meta'] = '_collections';
          } else {
            params['_DB'][collection] = 'all';
          }
        }
      } else if (pathArr[0].toUpperCase() === '/_FILE') {
        if (!params['_FILE']) {
          params['_FILE'] = {};
        }
        if (pathArr.length >= 3) {
          if (pathArr.length === 3) {
            // dir
            params['_FILE'][pathArr[1]] = '';
          } else {
            // file
            var filepath = pathArr[1] + '.' + pathArr[2];
            params['_FILE'][filepath] = {};
            params['_FILE'][filepath]['_path'] = '';
            params['_FILE'][filepath]['_body'] = '';
          }
        }
      }
    }
